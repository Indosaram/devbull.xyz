---
title: "Building the Fastest Python Logger in Rust — Part 2: The Deadlock War Story"
date: 2026-04-14
tags: [rust, python, pyo3, gil, logging, performance, deadlock]
summary: "A war story about building LogXide’s Rust core, chasing flaky GIL deadlocks in CI, and fixing five lock-ordering bugs."
images: ["/static/images/logxide/blog-part2-cover.png"]
authors: ['default']
---

<TOCInline toc={props.toc} asDisclosure />

![Part 2 cover](/static/images/logxide/blog-part2-cover.png)
...
This was LogXide, my Rust-powered drop-in replacement for Python's logging module. I'd spent weeks making it 2.7x faster than stdlib. The benchmarks were beautiful. Then I merged to main, and everything caught fire.

If you haven't read Part 1, it covers what LogXide actually does: you `pip install logxide`, replace `import logging` with `from logxide import logging`, and suddenly your log calls drop from ~6 microseconds to ~2.2 microseconds. Same API, same handlers, same everything — just Rust underneath. It works with Flask, Django, FastAPI, Sentry, OTLP. That's the happy path. This is the story of how I almost never got there.
...
Here's how it shook out against the alternatives:

| Scenario | LogXide | Picologging (C) | stdlib | vs stdlib |
|----------|---------|-----------------|--------|-----------|
| Simple | 446K ops/s | 372K ops/s | 157K ops/s | **2.8x** |
| Structured | 412K ops/s | 357K ops/s | 154K ops/s | **2.7x** |
| Error | 426K ops/s | 361K ops/s | 155K ops/s | **2.7x** |

The 20% edge over picologging? Picologging reimplements logging in C but still calls back into Python for I/O. LogXide's handlers are pure Rust end to end.
